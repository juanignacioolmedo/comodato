...
def confirm_and_send(treeview):
    confirmacion = messagebox.askyesno(
        "Confirmar acción", "¿Está seguro que desea enviar estos cambios?"
    )
...
def execute_insert(treeview):
    ...
    try:
        ...
        if successful_records == total_records:
            messagebox.showinfo(
                "",
                f"✅ Todos los registros se insertaron correctamente ({successful_records}/{total_records})",
            )
        else:
            messagebox.showinfo(
                "",
                f"⚠️ Solo se insertaron {successful_records} de {total_records} registros",
            )
...
def verify_inserts(id_cliente, id_producto):
    ...
    if result:
        ...
        for row in result:
            print(
                f"Tipo: {row[0]}, Cliente: {row[1]}, Producto: {row[2]}, Cantidad: {row[3]}, Fecha: {row[4]}"
            )
    else:
...
def setup_window():
    ...
    try:
        ...
        if not bdweb:
            raise ValueError(
                "No se pudo obtener la configuración de la base de datos desde H2O.ini"
            )
        ...
        if not sql_params:
            raise ValueError(
                "No se pudieron obtener los parámetros de conexión desde serviceairtech."
            )
        ...
        messagebox.showerror(
            ...
            f"Error al configurar la conexión a la base de datos: {e}",
        )
        return root
    except Exception as e:
        messagebox.showerror(
            "Error inesperado", f"Error inesperado al configurar la conexión: {e}"
        )
        return root
    # Crear los filtros con el callback para detectar cambios
    reparto_listbox, _ = create_multi_select(
        filter_container,
        "Repartos:",
        fetch_repartos,
        lambda event: on_filter_change(
            event, reparto_listbox, producto_listbox, btn_new
        ),
    )
    producto_listbox, _ = create_multi_select(
        filter_container,
        "Productos:",
        fetch_productos,
        lambda event: on_filter_change(
    ...
    btn_apply = tk.Button(
        ...
        command=lambda: apply_filters(
            treeview, reparto_listbox, producto_listbox, btn_new
        ),
        width=20,
